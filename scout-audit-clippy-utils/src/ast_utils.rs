...
pub fn eq_pat(l: &Pat, r: &Pat) -> bool {
    ...
    match (&l.kind, &r.kind) {
        ...
        (Ident(b1, i1, s1), Ident(b2, i2, s2)) => {
            b1 == b2 && eq_id(*i1, *i2) && both(s1, s2, |l, r| eq_pat(l, r))
        }
...
pub fn eq_generic_args(l: &GenericArgs, r: &GenericArgs) -> bool {
    match (l, r) {
        (GenericArgs::AngleBracketed(l), GenericArgs::AngleBracketed(r)) => {
            over(&l.args, &r.args, eq_angle_arg)
        }
...
pub fn eq_angle_arg(l: &AngleBracketedArg, r: &AngleBracketedArg) -> bool {
    match (l, r) {
        ...
        (AngleBracketedArg::Constraint(l), AngleBracketedArg::Constraint(r)) => {
            eq_assoc_constraint(l, r)
        }
        _ => false,
    }
}
...
pub fn eq_expr(l: &Expr, r: &Expr) -> bool {
    ...
    match (&l.kind, &r.kind) {
        ...
        (Binary(lo, ll, lr), Binary(ro, rl, rr)) => {
            lo.node == ro.node && eq_expr(ll, rl) && eq_expr(lr, rr)
        }
        (Unary(lo, l), Unary(ro, r)) => {
            mem::discriminant(lo) == mem::discriminant(ro) && eq_expr(l, r)
        }
        (Lit(l), Lit(r)) => l == r,
        (Cast(l, lt), Cast(r, rt)) | (Type(l, lt), Type(r, rt)) => eq_expr(l, r) && eq_ty(lt, rt),
        (Let(lp, le, _, _), Let(rp, re, _, _)) => eq_pat(lp, rp) && eq_expr(le, re),
        (If(lc, lt, le), If(rc, rt, re)) => {
            eq_expr(lc, rc) && eq_block(lt, rt) && eq_expr_opt(le, re)
        }
        (While(lc, lt, ll), While(rc, rt, rl)) => {
            eq_label(ll, rl) && eq_expr(lc, rc) && eq_block(lt, rt)
        }
        ...
        (AssignOp(lo, lp, lv), AssignOp(ro, rp, rv)) => {
            lo.node == ro.node && eq_expr(lp, rp) && eq_expr(lv, rv)
        }
        ...
        (Range(lf, lt, ll), Range(rf, rt, rl)) => {
            ll == rl && eq_expr_opt(lf, rf) && eq_expr_opt(lt, rt)
        }
...
pub fn eq_item_kind(l: &ItemKind, r: &ItemKind) -> bool {
    ...
    match (l, r) {
            ...
            lu == ru
                && match (lmk, rmk) {
                    (ModKind::Loaded(litems, linline, _), ModKind::Loaded(ritems, rinline, _)) => {
                        linline == rinline
                            && over(litems, ritems, |l, r| eq_item(l, r, eq_item_kind))
                    }
        ...
        (ForeignMod(l), ForeignMod(r)) => {
            both(&l.abi, &r.abi, eq_str_lit)
                && over(&l.items, &r.items, |l, r| {
                    eq_item(l, r, eq_foreign_item_kind)
                })
        }
        (
        ...
        (Enum(le, lg), Enum(re, rg)) => {
            over(&le.variants, &re.variants, eq_variant) && eq_generics(lg, rg)
        }
        ...
        (TraitAlias(lg, lb), TraitAlias(rg, rb)) => {
        ...
        (MacCall(l), MacCall(r)) => eq_mac_call(l, r),
        (MacroDef(l), MacroDef(r)) => {
            l.macro_rules == r.macro_rules && eq_delim_args(&l.body, &r.body)
        }
        _ => false,
    }
}
...
pub fn eq_variant(l: &Variant, r: &Variant) -> bool {
    l.is_placeholder == r.is_placeholder
        ...
        && both(&l.disr_expr, &r.disr_expr, |l, r| {
...
pub fn eq_generics(l: &Generics, r: &Generics) -> bool {
    over(&l.params, &r.params, eq_generic_param)
        && over(
            &l.where_clause.predicates,
            &r.where_clause.predicates,
            |l, r| eq_where_predicate(l, r),
        )
}
...
pub fn eq_where_predicate(l: &WherePredicate, r: &WherePredicate) -> bool {
    ...
    match (l, r) {
        ...
        (RegionPredicate(l), RegionPredicate(r)) => {
            eq_id(l.lifetime.ident, r.lifetime.ident)
                && over(&l.bounds, &r.bounds, eq_generic_bound)
        }
        (EqPredicate(l), EqPredicate(r)) => {
            eq_ty(&l.lhs_ty, &r.lhs_ty) && eq_ty(&l.rhs_ty, &r.rhs_ty)
        }
        _ => false,
    }
}
...
pub fn eq_closure_binder(l: &ClosureBinder, r: &ClosureBinder) -> bool {
    match (l, r) {
        ...
        (
            ...
            lp.len() == rp.len()
                && std::iter::zip(lp.iter(), rp.iter()).all(|(l, r)| eq_generic_param(l, r))
        }
        _ => false,
    }
}
...
pub fn eq_ty(l: &Ty, r: &Ty) -> bool {
    ...
    match (&l.kind, &r.kind) {
        ...
        | (Err, Err)
        | (CVarArgs, CVarArgs) => true,
        (Slice(l), Slice(r)) => eq_ty(l, r),
        (Array(le, ls), Array(re, rs)) => eq_ty(le, re) && eq_expr(&ls.value, &rs.value),
        (Ptr(l), Ptr(r)) => l.mutbl == r.mutbl && eq_ty(&l.ty, &r.ty),
        (Ref(ll, l), Ref(rl, r)) => {
            both(ll, rl, |l, r| eq_id(l.ident, r.ident))
                && l.mutbl == r.mutbl
                && eq_ty(&l.ty, &r.ty)
        }
        (BareFn(l), BareFn(r)) => {
