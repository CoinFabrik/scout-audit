...
use rustc_hir::{
    BinOp, BinOpKind, Block, ConstBlock, Expr, ExprKind, HirId, Item, ItemKind, Node, QPath, UnOp,
};
    ...
    pub fn expr(&mut self, e: &Expr<'_>) -> Option<Constant<'tcx>> {
        match e.kind {
            ExprKind::ConstBlock(ConstBlock { body, .. }) => {
                self.expr(self.lcx.tcx.hir().body(body).value)
            }
            ExprKind::DropTemps(e) => self.expr(e),
            ExprKind::Path(ref qpath) => {
                self.fetch_path(qpath, e.hir_id, self.typeck_results.expr_ty(e))
            }
            ...
            ExprKind::Lit(lit) => {
                if is_direct_expn_of(e.span, "cfg").is_some() {
                    ...
                    Some(lit_to_mir_constant(
                        &lit.node,
                        self.typeck_results.expr_ty_opt(e),
                    ))
                }
            }
            ...
            ExprKind::Field(local_expr, ref field) => {
                ...
                if let Some(Constant::Adt(constant)) = &self.expr(local_expr)
                    ...
                    && adt_def.is_struct()
                    && let Some(desired_field) =
                        field_of_struct(*adt_def, self.lcx, *constant, field)
                {
        ...
        match res {
            Res::Def(DefKind::Const | DefKind::AssocConst, def_id) => {
                ...
                let result = self
                    ...
                    .const_eval_resolve(
                        self.param_env,
                        mir::UnevaluatedConst::new(def_id, args),
                        None,
                    )
                    .ok()
            ...
            if span.ctxt == SyntaxContext::root() {
                    ...
                    && let Some(src) = src.as_str()
                {
                    use rustc_lexer::TokenKind::{
                        BlockComment, LineComment, OpenBrace, Semi, Whitespace,
                    };
                    if !tokenize(src)
                        .map(|t| t.kind)
                        .filter(|t| {
    ...
    fn binop(&mut self, op: BinOp, left: &Expr<'_>, right: &Expr<'_>) -> Option<Constant<'tcx>> {
        ...
        match (l, r) {
            (Constant::Int(l), Some(Constant::Int(r))) => {
                match *self.typeck_results.expr_ty_opt(left)?.kind() {
                    ty::Int(ity) => {
...
fn field_of_struct<'tcx>(
    ...
    if let mir::Const::Val(result, ty) = result
        && let Some(dc) = lcx
            .tcx
            .try_destructure_mir_constant_for_diagnostics(result, ty)
