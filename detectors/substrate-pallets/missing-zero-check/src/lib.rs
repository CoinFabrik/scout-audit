...
use common::{
    analysis::{
        decomposers::{
            expr_to_call, expr_to_method_call, expr_to_path, path_to_resolved, resolved_to_def,
        },
        get_node_type_opt, get_receiver_ident_name,
    },
...
use rustc_hir::{
    ...
    BinOpKind, Expr, ExprKind, PatKind, QPath,
};
    ...
    fn check_fn(
        ...
        for (arg, hir_param) in mir_body.args_iter().zip(body.params.iter()) {
            ...
            if let TyKind::Alias(_alias_kind, alias_ty) = mir_body.local_decls[arg].ty.kind() {
                ...
                if def_id.contains("traits::Currency::Balance") {
                    let mut param_name = "";
        ...
        utf_storage.param_infos.clone().iter().for_each(|p| {
            if p.is_checked {
