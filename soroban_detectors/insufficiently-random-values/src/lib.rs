...
use scout_audit_internal::{SorobanDetector, DetectorImpl, SOROBAN_INSUFFICIENTLY_RANDOM_VALUES_LINT_MESSAGE};
...
impl<'tcx> LateLintPass<'tcx> for InsufficientlyRandomValues {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'_>) {
        if_chain! {
            ...
            then {
                SorobanDetector::InsufficientlyRandomValues.span_lint_and_help(
                    cx,
