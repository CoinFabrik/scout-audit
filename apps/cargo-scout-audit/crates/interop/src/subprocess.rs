...
use scout::scout::nightly_runner::set_up_environment;
#[cfg(not(windows))]
use std::process::Command;
use util::print::print_full_error;

pub fn run_subprocess<I: Serialize, O: DeserializeOwned>(toolchain: &String, exec: &PathBuf, input: &I) -> Result<O>{
    ...
    let output = {
        let mut file = File::open(&output_path)
            .with_context(|| "Failed to open output temporary file")?;
        let mut s = String::new();
        file.read_to_string(&mut s)
            .with_context(|| format!("Error while reading {}", output_path))?;

        serde_json::from_str::<O>(&s).or_else(|_| Err(anyhow!("Failed to parse output file")))
    }?;
...
fn get_paths<I: Serialize>(input: &I) -> Result<(String, String)>{
    let input_path = {
        let (mut file, path) = util::temp_path()
    ...
    let output_path = util::temp_path()
        .with_context(|| "Failed to create temporary file")?.1;

    Ok((input_path, output_path))
}

#[cfg(not(windows))]
fn do_it(exec: &PathBuf, environment: HashMap<String, String>, input_path: &String, output_path: &String) -> Result<()>{
...
fn subprocess_wrapper_with_result<I: DeserializeOwned, O: Serialize, F: FnOnce(&I) -> O>(f: F) -> Result<()>{
    ...
    let i = {
        ...
        serde_json::from_str::<I>(&s).or_else(|_| Err(anyhow!("Failed to parse input data")))?
    };
...

    {
        ...
        let serialized = serde_json::to_string(&result).with_context(|| "Failed to serialize output data")?;
        let _ = file.write(serialized.as_bytes()).with_context(|| "Failed to write output data")?;
    }
