...
use cargo::{core::{Dependency, GitReference, SourceId, Verbosity}, GlobalContext};
...
pub fn call(path: &PathBuf, url: &str, output_path: &str) -> Result<bool>{
    ...
    let generate_pdf: Symbol<GeneratePdfFunc> = unsafe {
        lib.get(b"generate_pdf")
    }.with_context(|| "Failed to load PDF generator")?;
    ...
    Ok(unsafe{
        generate_pdf(urlp, output_pathp)
    })
}
