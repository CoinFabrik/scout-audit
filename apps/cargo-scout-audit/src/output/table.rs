...
impl Row {
    ...
    pub fn get<'a>(&'a self, i: usize) -> &'a Cell{
        &self.cells[i]
    }
    pub fn get_mut<'a>(&'a mut self, i: usize) -> &'a mut Cell{
        &mut self.cells[i]
    }
    ...
    pub fn to_json_array(&self, widths: &Vec<usize>) -> Value{
        Value::Array(self.to_json_array2(widths))
    }
    pub fn to_json_array2(&self, widths: &Vec<usize>) -> Vec<Value>{
    ...
    pub fn to_json_object(&self, widths: &Vec<usize>) -> Value{
        let mut ret = serde_json::Map::<String, Value>::new();
        for (i, cell) in self.cells.iter().enumerate(){
...
impl Table{
    ...
    pub fn add_row(&mut self, row: Row) -> Result<&mut Row, ()>{
    ...
    fn print_line(widths: &Vec<usize>){
    ...
    fn print_row(row: &Row, widths: &Vec<usize>){
        for (index, cell) in row.iter().enumerate(){
            ...
            let string = {
                ...
                formatter
                    .text_str(&cell.pad(widths[index]))
                    .print()
            };
            print!("{}", string);
            print!(" ");
    ...
    pub fn to_json_map(&self) -> serde_json::Map::<String, Value>{
        ...
        let mut ret = serde_json::Map::<String, Value>::new();
        ret.insert("widths".into(), Value::Array(widths.iter().map(|x| Value::Number((*x).into())).collect()));

        ret.insert("header_order".into(), self.header.iter().map(|x| x.content.clone()).collect());
        ret.insert("header".into(), self.header.to_json_object(&widths));

        ret.insert("rows".into(), {
            let all_rows: Vec<_> = self.rows
                .iter()
    ...
    pub fn get<'a>(&'a self, i: usize) -> &'a Row{
        &self.rows[i]
    }
    pub fn get_mut<'a>(&'a mut self, i: usize) -> &'a mut Row{
        &mut self.rows[i]
    }
}

static mut COLOR_MAP: once_cell::sync::Lazy::<Mutex<HashMap<String, Color>>> = once_cell::sync::Lazy::new(|| {
...
fn get_color(o: &serde_json::Map<String, Value>) -> Color{
    if let Some(Value::String(color)) = o.get("color"){
        ...
        if let Some(Value::String(importance)) = o.get("importance"){
...
fn process_object_console(o: &serde_json::Map<String, Value>, pad: bool) -> String{
    ...
    if pad{
        if let Some(w) = o.get("w"){
            let w: usize = usize::try_from(json_to_int(w)).ok().or(Some(0)).unwrap();
            while padded.len() < w{
                padded.push(' ');
            }
        }
...
fn process_object_md(o: &serde_json::Map<String, Value>, pad: bool) -> String{
    ...
    padded = match color {
        ...
        _ => format!("<span style=\"color:{}\">{}</span>", color.to_string(), padded),
    };
    
    padded
}

fn filter_cell_wrapper<F: FnOnce(&serde_json::Map<String, Value>) -> String>(values: &HashMap<String, Value>, cb: F) -> Result<Value,tera::Error>{
...
fn process_order(values: &HashMap<String, Value>, header: &Value) -> Result<Vec<usize>, ()>{
    ...
    let order = if let Some(Value::Array(o)) = values.get("order"){
        o.iter().map(|x| json_to_string(x)).collect()
    }else{
...
pub(crate) fn prepare_tera_for_table_render_console(tera: &mut Tera, context: &mut Context, table: &Value, name: &str){
    register_functions_for_tera_console(tera);
    context.insert(name, table);
}

fn count_findings(findings: &[Value], crate_to_find: &String, detectors_info: &HashMap<String, LintInfo>) -> [usize; 4]{
...
pub(crate) fn construct_table(findings: &[Value], crates: &HashMap<String, bool>, detectors_info: &HashMap<String, LintInfo>) -> Table{
    let mut header = Row::from_strs(&["Crate", "Status", "Critical", "Medium", "Minor", "Enhancement"]);
    ...
    for krate in crate_order.iter(){
        ...
        let success_string = if success { "Analyzed" } else { "Compilation errors" }.to_string();
