...
pub const SOROBAN_DOS_UNBOUNDED_OPERATION_LINT_MESSAGE: &str ="In order to prevent a single transaction from consuming all the gas in a block, unbounded operations must be avoided";
pub const SOROBAN_INSUFFICIENTLY_RANDOM_VALUES_LINT_MESSAGE: &str ="Use env.prng() to generate random numbers, and remember that all random numbers are under the control of validators";
...
pub const SOROBAN_UNPROTECTED_MAPPING_OPERATION_LINT_MESSAGE: &str = "This mapping operation is called without access control on a different key than the caller's address";
...
pub const SOROBAN_UNUSED_RETURN_ENUM_LINT_MESSAGE : &str = "If any of the variants (Ok/Err) is not used, the code could be simplified or it could imply a bug";
