    ...
    ($(#[$attr:meta])* $vis:vis $NAME:ident, $Level:ident, $desc:expr, $register_pass_method:ident, $pass:expr, $info: tt) => {
        ...
        $crate::__maybe_mangle! {
            ...
            pub fn register_lints(sess: &rustc_session::Session, lint_store: &mut rustc_lint::LintStore) {
                ...
                let config_table = $crate::config_toml(stringify!($NAME));
                if ! $crate::check_disabled(&config_table) {
                    lint_store.register_lints(&[$NAME]);
...
pub fn check_disabled(res_opt_val: &ConfigResult<Option<toml::Value>>) -> bool {
    let mut disabled = false;
    if let Ok(opt_val) = res_opt_val {
        if let Some(val) = opt_val {
            if let toml::Value::Table(table) = val {
                let opt_disabled = table.get("disabled");
                if let Some(val_disabled) = opt_disabled {
                    if let toml::Value::Boolean(val) = val_disabled {
                        disabled = *val;
                    }
                }
            }
    ...
    let value = if let Ok(value) = std::env::var(env::DYLINT_TOML) {
        ...
        match result {
            ...
            _ => {
                ...
                let dylint_toml = workspace_root.join("scout.toml");
