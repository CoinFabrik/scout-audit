...
RUN rustup target add wasm32v1-none --toolchain nightly-2025-09-18
RUN rustup target add wasm32-unknown-unknown --toolchain nightly-2025-09-18

WORKDIR /usr/src/scout-audit/nightly/2025-09-18/detectors/ink
RUN cargo build --release
WORKDIR /usr/src/scout-audit/nightly/2025-09-18/detectors/rust
RUN cargo build --release
WORKDIR /usr/src/scout-audit/nightly/2025-09-18/detectors/soroban
RUN cargo build --release
WORKDIR /usr/src/scout-audit/nightly/2025-09-18/detectors/substrate-pallets
RUN cargo build --release
