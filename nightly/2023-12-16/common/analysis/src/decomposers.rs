...
use rustc_ast::{BindingAnnotation, Label, LitIntType, LitKind, UnOp};
use rustc_hir::{
    def::{DefKind, Res},
    Block, Expr, ExprField, ExprKind, HirId, LangItem, Local, LoopSource, MatchSource, Pat,
...
pub fn stmt_to_let<'hir>(kind: &'hir StmtKind<'hir>) -> Option<&'hir Local<'hir>> {
...
pub fn pattern_to_binding<'hir>(
    pat: &'hir PatKind<'hir>,
) -> Option<(&BindingAnnotation, &HirId, &Ident, &Option<&'hir Pat<'hir>>)> {
