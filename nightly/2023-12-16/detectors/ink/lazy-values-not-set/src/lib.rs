...
impl<'tcx> LateLintPass<'tcx> for LazyValuesNotSet {
    fn check_fn(
        ...
        for val in hm.values() {
            clippy_utils::diagnostics::span_lint(cx, LAZY_VALUES_NOT_SET, *val, LINT_MESSAGE);
        }
    }
}

fn clean_local_upwards(
    local: Local,
    hm: &HashMap<Local, Vec<Local>>,
...
impl LazyValuesNotSet {
    fn get_func_info(
        ...
        for local in locals_to_clean.clone() {
            locals_to_clean.extend(clean_local_upwards(
                local,
                &locals_dependencies,
                &mut HashSet::new(),
            ));
