...
const STORAGE_GET_METHODS: [&str; 2] = ["get", "get_unchecked"];
const UNWRAP_METHODS: [&str; 5] = [
    "unwrap",
    "expect",
    "unwrap_or",
    "unwrap_or_else",
...
pub fn is_unwrap_method(name: Symbol) -> bool {
    UNWRAP_METHODS.contains(&name.as_str())
}

pub fn is_get_method(name: Symbol) -> bool {
    STORAGE_GET_METHODS.contains(&name.as_str())
}
