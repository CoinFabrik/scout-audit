...
fn it_works_for_optional_value() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Example::multiply_dummy(RuntimeOrigin::signed(1), val2, 1));
        assert_eq!(Dummy::<Test>::get(), Some(val1 * val2));

        // Check that accumulate works when we Dummy has None in it.
        <Example as OnInitialize<u64>>::on_initialize(2);
        assert_ok!(Example::multiply_dummy(RuntimeOrigin::signed(1), val1, 1));
...
fn weights_work() {
    ...
    let default_call = pallet_example_basic::Call::<Test>::multiply_dummy {
        factor: 10,
        value: 1,
    };
